program fits_coaddspectra_e2ds
  ...
  do while (read_iostat.eq.0)
     ...
     write(*,*) ' BERV = ', berv , 'RV = ', rv_fin, 'CCD_GAIN = ',ccd_gain
     ...
     !! Rebinning of the individual orders. We keep track of starting and ending points of each order in the rebinned solution
     b_rb_pix = 0._PR
     s_rb_pix = 0._PR
     do ii=1,y_axis
        ...
        i = is(ii)
        j = ie(ii)
        !! write(*,*) i, j, r_pix(     1,ii),'->',r_rb_pix( i), '  and  ',r_pix(x_axis,ii),'->',r_rb_pix(j)
        ...
        !!write(*,*) i, j, s_pix(     1,ii),'->',b_rb_pix( i,ii), '  and  ',s_pix(x_axis,ii),'->',b_rb_pix(j,ii)
        call rebin_keepflux(r_pix(:,ii),dw_pix(:,ii),s_pix(:,ii),r_rb_pix(i:j),dw_rb_pix(i:j),s_rb_pix(i:j,ii),no_norm=1)
        !!write(*,*) i, j, s_pix(     1,ii),'->',s_rb_pix( i,ii), '  and  ',s_pix(x_axis,ii),'->',s_rb_pix(j,ii)
        !!write(*,*)
     end do
     ...
     do ii=2,y_axis
        ...
        !! write(*,*) ii,  j_is, j_ie, s_rb_pix(j_is,ii), s_rb_pix(j_ie,ii), b_rb_pix(j_is,ii)
        !! write(*,*) ii,  j_is, j_ie, j_rb_pix(j_is), j_rb_pix(j_ie), b_rb_pix(j_is,ii)
        !! write(*,*)
     end do
...
!!$              if (r_pix(i,j).lt.3700 .or. r_pix(i,j).gt.3910) cycle
              if (r_pix(i,j).lt.5000 .or. r_pix(i,j).gt.5800) cycle
...
!!$           if (r_rb_pix(i).lt.3700 .or. r_rb_pix(i).gt.3910) cycle
           if (r_rb_pix(i).lt.5000 .or. r_rb_pix(i).gt.5800) cycle
