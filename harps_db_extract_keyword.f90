...
!!$     write(nc,'(I1)') 1 ; sts = 0
     ...
     call FTGKYD(lun_fits,'HIERARCH ESO DRS BIS SPAN'     ,bis_span     ,comment,sts);  sts = 0 !Bisector velocity span [km/s]
     call FTGKYD(lun_fits,'HIERARCH ESO DRS BIS RV'       ,bis_rv       ,comment,sts);  sts = 0 !Bisector mean velocity [km/s]
     call FTGKYJ(lun_fits,'HIERARCH ESO DRS CCF MAXCPP'   ,ccf_maxccp   ,comment,sts);  sts = 0 !max count/pixel of CCF (e-)
     call FTGKYD(lun_fits,'HIERARCH ESO DRS CCF CONTRAST' ,ccf_contrast ,comment,sts);  sts = 0 !Contrast of  CCF (%)
     call FTGKYD(lun_fits,'HIERARCH ESO DRS CCF RV'       ,ccf_rv       ,comment,sts);  sts = 0 !Baryc RV (no drift correction) (km
     call FTGKYD(lun_fits,'HIERARCH ESO DRS CCF FWHM'     ,ccf_fwhm     ,comment,sts);  sts = 0 !FWHM of CCF (km/s)
     call FTGKYD(lun_fits,'HIERARCH ESO DRS CCF RVC'      ,ccf_rvc      ,comment,sts);  sts = 0 !Baryc RV (drift corrected) (km/s)
     call FTGKYJ(lun_fits,'HIERARCH ESO DRS CCF LINES'    ,ccf_lines    ,comment,sts);  sts = 0 !nr lines used
     call FTGKYS(lun_fits,'HIERARCH ESO DRS CCF MASK'     ,ccf_mask     ,comment,sts);  sts = 0 !Mask filename
     call FTGKYD(lun_fits,'HIERARCH ESO DRS CCF NOISE'    ,ccf_noise    ,comment,sts);  sts = 0 !Photon noise on CCF RV (km/s)
...
!!$     call FTGKYL(lun_fits,'EXPSTART'                      ,expstart    ,comment,sts);  sts = 0 !Exposure start time (UT) (hh:mm:ss)
     ...
     call FTGKYD(lun_fits,'HIERARCH ESO DRS BERV'         ,berv         ,comment,sts);  sts = 0 !Barycentric Earth Radial Velocity
     call FTGKYD(lun_fits,'HIERARCH ESO DRS BJD'          ,bjd          ,comment,sts);  sts = 0 !Barycentric Julian Day
     call FTGKYD(lun_fits,'HIERARCH ESO DRS BERVMX'       ,bervmax      ,comment,sts);  sts = 0 !Maximum BERV

!! these are REAL single precision variables (if not differently specified)
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT SPE RV' ,drift_spe_rv ,comment,sts);  sts = 0 !Th RV Drift [m/s]
     call FTGKYJ(lun_fits,'HIERARCH ESO DRS DRIFT NBCOS'  ,drift_nbcos  ,comment,sts);  sts = 0 !Th Drift Nbr cosmic detected (integer)
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT RFLUX'  ,drift_rflux  ,comment,sts);  sts = 0 !Th Drift Flux ratio
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT NOISE'  ,drift_noise  ,comment,sts);  sts = 0 !Th Drift photon noise [m/s]
     !call FTGKYS(lun_fits,'HIERARCH ESO DRS DRIFT REF CCF',drift_ref_ccf,comment,sts);  sts = 0 !fits
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT REF RV' ,drift_ref_rv ,comment,sts);  sts = 0 !RV of Th ref CCF [m/s]
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT CCF RV' ,drift_ccf_rv ,comment,sts);  sts = 0 !CCF RV Drift [m/s]
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT RV USED',drift_rv_used,comment,sts);  sts = 0 !Used RV Drift [m/s]
     !call FTGKYS(lun_fits,'HIERARCH ESO DRS DRIFT ALGO'   ,drift_algo   ,comment,sts);  sts = 0 !Algo used for Drift meas. (tex)
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DRIFT QC'     ,drift_qc     ,comment,sts);  sts = 0 !QC on drift (text)
     call FTGKYE(lun_fits,'HIERARCH ESO DRS DVRMS'        ,ddvrms       ,comment,sts);  sts = 0 !Estimated RV uncertainty [m/s]

     call FTGKYE(lun_fits,'HIERARCH ESO DRS SPE EXT SN1'   ,sn1         ,comment,sts);  sts = 0 !S_N order center1
     call FTGKYE(lun_fits,'HIERARCH ESO DRS SPE EXT SN46' ,sn46         ,comment,sts);  sts = 0 !S_N order center46
     call FTGKYE(lun_fits,'HIERARCH ESO DRS SPE EXT SN68' ,sn68         ,comment,sts);  sts = 0 !S_N order center68
     call FTGKYE(lun_fits,'HIERARCH ESO TEL TARG MAG'     ,tel_mag      ,comment,sts);  sts = 0
